...
class StatManager():
    ...
    def __init__(self):
        ...
        self.osis = j.core.osis.getCategory(self.osisclient, 'stats', 'history')
    ...
    def getHistoryObject(self, id):
        ...
        #         key=j.base.time.get5MinuteId(now-nrsecondsago)
    ...
    def _getKey5min(self, epoch):
        # nrsecondsago=5*60
        return j.base.time.get5MinuteId(epoch)

    def addInfoLine2HistoryObj(self, id, value, epoch=None):
        if epoch == None:
            epoch = self.getEpoch()
        obj = self.getHistoryObject(id)
        key = j.base.time.get5MinuteId(epoch)
        key2 = j.base.time.getHourId(epoch)
    ...
    def getHeaders(self, start, stop):
        ...
        start2 = j.base.time.get5MinuteId(start)
        stop2 = j.base.time.get5MinuteId(stop) - start2
        start2 = 0
        hoursecs = 60 * 60 / 5
        r = []
    ...
    def getInfo5MinFromTo(self, id, start, stop):
        ...
        """
        obj = self.getHistoryObject(id)
        start2 = j.base.time.get5MinuteId(self.getTimeStamp(start))
        stop2 = j.base.time.get5MinuteId(self.getTimeStamp(stop))
        r = []
        for i in range(start2, stop2 + 1):
    ...
    def getInfo5Min(self, id, start=0, stop=0, epoch2human=False):
        ...
        if stop == 0:
            ...
            stop2 = j.base.time.get5MinuteId(stop)
        for key in range(stop2, start2, -1):
            epoch = j.base.time.fiveMinuteIdToEpoch(key)
            if epoch2human:
                epoch = j.base.time.epoch2HRDateTime(epoch)
