...
class GitFactory:
    ...
    def find(self,account=None,name=None,interactive=False,returnGitClient=False):
        """
        ...
        def _getRepos(codeDir,account=None,name=None):
            ...
            result=[]
            repos=[]
            for top in j.system.fs.listDirsInDir(codeDir, recursive=False, dirNameOnly=True, findDirectorySymlinks=True):
                for account in j.system.fs.listDirsInDir("%s/%s"%(j.dirs.codeDir,top), recursive=False, dirNameOnly=True, findDirectorySymlinks=True):
                    if checkaccount(account):
                        ...
                        if j.system.fs.exists(path="%s/.git"%accountdir):
                            ...
                            for reponame in j.system.fs.listDirsInDir("%s/%s/%s"%(j.dirs.codeDir,top,account), recursive=False, dirNameOnly=True, findDirectorySymlinks=True):
