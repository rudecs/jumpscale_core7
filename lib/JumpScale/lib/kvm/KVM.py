...
class KVM():
    ...
    def _get_rootpath(self,name):
        rootpath=j.system.fs.joinPaths(self.vmpath, '%s' % (name))
        return rootpath
    ...
    def loadImages(self):
        ...
        """
        for item in j.system.fs.listFilesInDir(self.imagepath, recursive=False, filter="*.hrd", listSymlinks=True):
